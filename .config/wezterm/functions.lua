...
function module.apply_to_config(config)
  ...
  table.insert(config.key_tables.setting_mode, { key = "9", action = act({ EmitEvent = "reset-color" }) })
  table.insert(config.key_tables.setting_mode, { key = "8", action = act({ EmitEvent = "toggle-color-scheme" }) })
end
